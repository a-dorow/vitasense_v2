function vitasenserecorderappv5
% Vitasense Recorder app: camera capture + metadata logging (AVI/MP4 toggle)
% After recording finishes, runs your pipeline wrapper on the recorded
% video.This one also has the BP integration. 
...
%% ------------- File/Folder Helpers -----------
    ...
    function repo_root = get_repo_root()
        % This file: ...\app\matlab\main\vitasenserecorderappv4.m
        this_file = mfilename('fullpath');
        this_dir = fileparts(this_file);
    ...
    for k = 1:numel(files)
        ...
        % Only allow your 4 expected models (strict control)
        allowed = ["alexnet","lstm","resnet","slapnicar"];
        if any(strcmp(shortName, allowed))
            displayNames{end+1} = char(shortName); 
...
function run_bp_quick_popup(video_path, subfolder)
    ...
    % Save MAT input next to video
    bp_mat = fullfile(subfolder, "bp_input.mat");
    try
        save(bp_mat, 'y', 't2', 'fs_in');
