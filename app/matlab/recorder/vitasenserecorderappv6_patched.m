function vitasenserecorderappv6_patched
...
% Row at y=55 for pipeline + bp controls
...
ui.runBP = uicheckbox(ui.ctrlpanel, ...
    'Text', 'Run BP', ...
    'Position', [360 55 70 22], ...
    'Value', true);

ui.bpModelLabel = uilabel(ui.ctrlpanel, ...
    'Text','BP Model:', ...
    'Position',[435 55 65 22]);
...
% Store model map here (populated by refresh_bp_models)
ui.bpModelMap = containers.Map;
...
% Initialize BP models (repo-local /models)
refresh_bp_models();
...
%% ------Nested UI Helpers------
    ...
    function refresh_bp_models()
        [names, map] = list_models();

        if isempty(names)
            ui.bpModelDropdown.Items = {'(none)'};
            ui.bpModelDropdown.Value = '(none)';
            ui.bpModelMap = containers.Map;
            logmsg("No BP models found in: " + string(get_model_dir()));
            return;
        end

        ui.bpModelDropdown.Items = names;

        % Deterministic default preference
        pref = 'alexnet';
        if any(strcmp(names, pref))
            ui.bpModelDropdown.Value = pref;
        else
            ui.bpModelDropdown.Value = names{1};
        end

        ui.bpModelMap = map;
        logmsg("BP models loaded: " + strjoin(string(names), ", "));
    end
    ...
    function startrecording(~,~)
        ...
        try
            ...
            try
                ...
                % BP: only in Quick Popup mode, kept out of research mode
                if doPopup
                    try
        ...
        function setRunUIEnabled(tf)
            ...
            ui.runBP.Enable           = onOff(tf);
            ui.bpModelDropdown.Enable = onOff(tf);
        end
    end
...
%% ------------- File/Folder Helpers -----------
    ...
    function s = defaultRoot()
        ...
        try
            this_file = mfilename('fullpath');    % ...\app\matlab\main\...
    ...
    function model_dir = get_model_dir()
        model_dir = fullfile(get_repo_root(), 'models');
    end

    function [displayNames, fileMap] = list_models()
        % Returns:
        %   displayNames = {'alexnet','lstm','resnet','slapnicar'}
        %   fileMap      = containers.Map(displayName -> full_path)
        ...
        files = dir(fullfile(model_dir, '*.h5'));
        if isempty(files)
            return;
        end
        ...
        for k = 1:numel(files)
            ...
            shortName = string(tokens{1});

            allowed = ["alexnet","lstm","resnet","slapnicar"];
            if any(strcmp(shortName, allowed))
                displayNames{end+1} = char(shortName); %#ok<AGROW>
                fileMap(char(shortName)) = fullfile(files(k).folder, fname);
            end
        end
    end
    ...
    function run_bp_quick_popup(video_path, subfolder)
        ...
        if ~logical(ui.runBP.Value)
            logmsg("BP disabled (Run BP unchecked).");
            return;
        end

        sel = string(ui.bpModelDropdown.Value);
        if sel == "(none)"
            logmsg("BP model not selected.");
            return;
        end
        if ~isKey(ui.bpModelMap, char(sel))
            logmsg("BP model missing from map: " + sel);
            return;
        end

        model_path = string(ui.bpModelMap(char(sel)));
        if ~isfile(model_path)
            logmsg("BP model file not found: " + model_path);
            return;
        end
        ...
        % Drop first 6 seconds
        start_idx = floor(6.0 * fs_in) + 1;
        if start_idx > N
            logmsg("BP: video too short after 6s skip.");
            return;
        end

        % Use GREEN channel mean (column 2)
        y = double(rawColorSignal(:,2));
        y = y(start_idx:end);

        % Remove NaNs (face not found early etc.)
        y = y(isfinite(y));

        if numel(y) < round(7.0 * fs_in)
            logmsg("BP: not enough samples for 7s window after skip.");
            return;
        end
        ...
        % Call Python server (local machine)
        server_url = "http://127.0.0.1:8000/predict_summary";

        % Quick connectivity check (optional but helpful for debugging)
        try
            health = webread("http://127.0.0.1:8000/health");
            if isstruct(health) && isfield(health,'ok') && ~health.ok
                logmsg("BP server /health returned ok=false.");
            end
        catch
            logmsg("BP server /health unreachable. Is uvicorn running?");
        end

        payload = struct();
        payload.input_path = char(bp_mat);
        payload.input_type = 'mat';
        payload.model_path = char(model_path);
        payload.win_s = 7.0;
        payload.stride_s = 1.0;

        opts = weboptions('MediaType','application/json','Timeout',120);

        % Debug: log outbound request
        try
            logmsg("Calling BP server -> " + string(server_url));
            logmsg("BP payload: input_type=" + string(payload.input_type) + ...
                   ", input_path=" + string(payload.input_path) + ...
                   ", model=" + string(payload.model_path));
        catch
        end

        try
            resp = webwrite(char(server_url), payload, opts);
        catch ME
            logErr(ME);
            try
                logmsg("BP server call failed: " + string(ME.message));
            catch
                logmsg("BP server call failed.");
            end
            return;
        end

        if ~isfield(resp,'sbp_mean') || ~isfield(resp,'dbp_mean')
            logmsg("BP server response missing sbp_mean/dbp_mean.");
            return;
        end

        sbp = round(double(resp.sbp_mean));
        ...
        try
            uialert(ui.fig, sprintf('%d/%d', sbp, dbp), 'BP Estimate');
        catch
            logmsg("BP estimate: " + string(sbp) + "/" + string(dbp));
        end
    end
