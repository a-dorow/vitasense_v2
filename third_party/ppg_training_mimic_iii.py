...
def ppg_train_mimic_iii(architecture,
    ...
    history = model.fit(
        train_dataset,
        steps_per_epoch=int(Ntrain)//batch_size, #edited and added an int since Ntrain is a float same as Nval and Ntest
        epochs=N_epochs,
    ...
    for i in range(int(Ntest//batch_size)):
        ...
        #test_results = test_results.append(TestBatchResult) this didn't work
        test_results = pd.concat([test_results, TestBatchResult], ignore_index=True)
